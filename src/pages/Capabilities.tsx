    ...
    <div className="min-h-screen pb-24 px-6 relative">
      ...
      <div className="max-w-3xl mx-auto pt-20 relative z-10">
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 1, ease }} className="text-center mb-20">
          ...
          <p className="text-[14px] text-muted-foreground/88 font-light max-w-md mx-auto leading-relaxed">
        ...
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.5 }} className="flex flex-wrap justify-center gap-x-8 gap-y-2 mb-16">
          {systemStats.map((stat, i) => (
            <motion.div key={stat.label} initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.7 + i * 0.08 }} className="text-center">
              <div className="text-lg font-extralight text-foreground/100 font-mono">{stat.value}</div>
              <div className="text-[9px] text-muted-foreground/88 tracking-wider uppercase">{stat.label}</div>
            </motion.div>
          ))}
        </motion.div>
        ...
        >
          <div className="flex items-center gap-2.5 mb-4">
            <Box className="w-3.5 h-3.5 text-muted-foreground/88" strokeWidth={1.5} />
            <span className="text-[10px] text-muted-foreground/92 tracking-[0.15em] uppercase font-mono">Composizione modulare</span>
          </div>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
            {moduleGroups.map((group, gi) => (
              ...
              >
                <div className="text-[9px] text-muted-foreground/90 font-light mb-2">{group.title}</div>
                <div className="space-y-1">
                  {group.items.map((item) => (
                    <div key={item} className="flex items-center gap-1.5">
                      <CheckCircle2 className="w-2 h-2 text-success/80 flex-shrink-0" />
                      <span className="text-[8px] text-muted-foreground/90 font-mono">{item}</span>
                    </div>
                  ))}
                </div>
              </motion.div>
            ...
            >
              <div className="flex items-start gap-4">
                ...
                <div className="flex-1 min-w-0">
                  <div className="text-[13px] font-light text-foreground/100 mb-1">{cap.label}</div>
                  <div className="text-[11px] text-muted-foreground/92 leading-relaxed font-light">{cap.desc}</div>
                </div>
              </div>
            </motion.div>
          ))}
        ...
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 1.2 }} className="mt-16 text-center">
          <div className="text-[9px] text-muted-foreground/88 tracking-[0.3em] uppercase mb-4">SORGENTI COLLEGATE</div>
          <div className="flex flex-wrap justify-center gap-4">
            {["WCA Network", "Contatti Import", "Business Card", "Company Reports", "CRM Core", "Deep Search", "Voice AI", "API Esterne"].map((src, i) => (
              <motion.span
                key={src}
                className="text-[10px] text-muted-foreground/88 font-light"
        ...
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 1.5 }} className="mt-12 text-center">
          <p className="text-[11px] text-muted-foreground/90 font-light max-w-sm mx-auto">
