        ...
        <div className={`flex-1 flex flex-col transition-all duration-700 ease-out ${canvas ? "max-w-[50%]" : ""}`}>
          {isEmpty ? (
            <div className="flex-1 flex flex-col items-center justify-center px-8">
              ...
              <motion.p initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.6 }} className="text-[13px] text-muted-foreground/50 font-light mb-10 text-center max-w-sm">
              ...
              <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.9 }} className="flex flex-col items-center gap-2">
                {quickPrompts.map((p, i) => (
                  <motion.button
                    ...
                    whileHover={{ x: 4 }}
                    className="text-[12px] px-4 py-2.5 rounded-2xl text-muted-foreground/45 hover:text-muted-foreground/70 hover:bg-secondary/[0.06] transition-all duration-700 text-left"
                  >
                    → {p}
                  </motion.button>
                ))}
              ...
              <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 1.8 }} className="flex flex-wrap justify-center gap-x-4 gap-y-1 mt-12">
                {["Source Unification", "Search Contacts", "Parse Cards", "Create Draft", "Send Batch", "Read Aloud", "Audit Action"].map((cap, i) => (
                  <motion.span key={cap} className="text-[9px] text-muted-foreground/30 font-light" animate={{ opacity: [0.2, 0.45, 0.2] }} transition={{ duration: 4, repeat: Infinity, delay: i * 0.5 }}>
                    {cap}
                  </motion.span>
                ))}
              </motion.div>
                    ...
                    {msg.thinking ? (
                      <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, scale: 0.95 }} transition={{ duration: 0.4, ease }} className="flex items-start gap-3">
                        ...
                        <div className="flex items-center gap-2 px-5 py-4">
                          ...
                          <span className="text-[11px] text-muted-foreground/40 ml-2 font-light">Attivo tool operativi...</span>
                        </div>
                      </motion.div>
                    ) : (
                      <motion.div
                      ...
                      >
                        ...
                        >
                          {msg.agentName && (
                            <motion.div initial={{ opacity: 0, x: -8 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.15 }} className="text-[9px] text-primary/55 font-mono mb-2.5 tracking-[0.2em] uppercase">
                              {msg.agentName}
                            </motion.div>
                          )}
            ...
            <div className="max-w-xl mx-auto">
              ...
              >
                <motion.button
                  ...
                  whileTap={{ scale: 0.9 }}
                  className={`w-9 h-9 rounded-xl flex items-center justify-center transition-all duration-500 flex-shrink-0 ${micActive ? "bg-primary/12 text-primary/70" : "text-muted-foreground/30 hover:text-muted-foreground/50"}`}
                >
                ...
                <input
                  ...
                  className="flex-1 bg-transparent text-[14px] outline-none placeholder:text-muted-foreground/35 font-light text-foreground/90"
                />
                <motion.button
                ...
                <motion.button onClick={() => sendMessage()} disabled={!input.trim()} whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.9 }} className="w-9 h-9 rounded-xl flex items-center justify-center bg-primary/10 text-primary/55 hover:bg-primary/15 hover:text-primary/80 transition-all duration-500 disabled:opacity-20">
