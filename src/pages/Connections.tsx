  ...
  return (
    <div className="min-h-screen pb-24 flex flex-col items-center px-6 pt-16">
      ...
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8, ease }} className="text-center mb-16 relative z-10">
        ...
        <p className="text-sm text-muted-foreground/88 font-light">Multi-source intelligence · 14 origini attive · 1.8M record unificati</p>
      </motion.div>
      ...
      <div className="w-full max-w-2xl space-y-10 relative z-10 mb-16">
        {sourceGroups.map((group, gi) => (
          ...
          >
            <div className="text-[9px] text-muted-foreground/90 tracking-[0.3em] uppercase mb-3 px-1">{group.title}</div>
              ...
              {group.sources.map((src, si) => (
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <span className="text-xs text-muted-foreground/90 ml-2">{src.records}</span>
                  </div>
      ...
      >
        <div className="text-[9px] text-muted-foreground/88 tracking-[0.3em] uppercase mb-3">SOURCE UNIFICATION</div>
        <p className="text-[12px] text-muted-foreground/92 font-light leading-relaxed">
      ...
      <div className="w-full max-w-2xl relative z-10">
        <div className="text-[9px] text-muted-foreground/88 tracking-[0.3em] uppercase mb-3 px-1">CONNETTORI INFRASTRUTTURA</div>
        <div className="space-y-1.5">
          {connections.map((conn, i) => (
            ...
            >
              <conn.icon className={`w-3.5 h-3.5 flex-shrink-0 ${conn.status === "connected" ? "text-primary/60" : "text-warning/92"}`} strokeWidth={1.5} />
              <div className="flex-1 min-w-0">
                <span className="text-[13px] font-light text-foreground/100">{conn.name}</span>
                <span className="text-[10px] text-muted-foreground/88 ml-2">{conn.desc}</span>
              </div>
              <div className="flex items-center gap-3">
                {conn.records && <span className="text-[9px] text-muted-foreground/88 font-mono">{conn.records}</span>}
