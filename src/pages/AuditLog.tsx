...
const statusColor: Record<Status, string> = {
  suggested: "text-primary/96", previewed: "text-info/96", approved: "text-success/95", executed: "text-success/95", error: "text-destructive/96", policy: "text-accent/96",
};
  ...
  return (
    <div className="min-h-screen pb-24 flex flex-col items-center px-6 pt-16">
      ...
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8, ease }} className="text-center mb-12 relative z-10">
        ...
        <p className="text-sm text-muted-foreground/98 font-light">Ogni azione tracciata</p>
      </motion.div>
      ...
      <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.2 }} className="w-full max-w-2xl mb-6 relative z-10">
        <div className="float-panel-subtle px-4 py-3 flex items-center gap-3">
          <Search className="w-3.5 h-3.5 text-muted-foreground/98" />
        ...
        <div className="space-y-0.5">
          {entries.map((entry, i) => {
            ...
            return (
              ...
              >
                <span className="text-[10px] font-mono text-muted-foreground/98 w-10">{entry.time}</span>
                ...
                <span className="text-[10px] text-muted-foreground/98">{entry.agent}</span>
              </motion.div>
            );
          })}
        </div>
