    ...
    <div className="min-h-screen pb-32 px-6 relative">
      ...
      <div className="max-w-4xl mx-auto pt-20 relative z-10">
        ...
        <motion.div initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 1.2, ease }} className="text-center mb-24">
          ...
          <p className="text-[14px] text-muted-foreground/100 font-light max-w-lg mx-auto leading-relaxed mb-8">
          ...
          >
            <div className="text-center">
              <div className="text-[9px] text-muted-foreground/100 tracking-[0.2em] uppercase mb-2 font-mono">SUPERFICIE</div>
              <div className="text-[13px] text-foreground/100 font-light">Prompt · Assistant · Canvas</div>
              <div className="text-[10px] text-muted-foreground/100 mt-1">L'utente vede solo questo</div>
            </div>
            ...
            <div className="text-center">
              <div className="text-[9px] text-muted-foreground/100 tracking-[0.2em] uppercase mb-2 font-mono">MOTORE</div>
              <div className="text-[13px] text-foreground/100 font-light">10 layer · 13 tool · 9 moduli</div>
              <div className="text-[10px] text-muted-foreground/100 mt-1">L'utente non lo vede mai</div>
            </div>
          </motion.div>
        </motion.div>
        ...
        <motion.div
          ...
          className="mb-24"
        >
          <div className="text-[9px] text-muted-foreground/100 tracking-[0.3em] uppercase text-center mb-6 font-mono">FLUSSO ARCHITETTURALE</div>
          <div className="flex items-center justify-center gap-1 flex-wrap">
            {flowSteps.map((step, i) => (
              <motion.div key={step} className="flex items-center gap-1" initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 1 + i * 0.1, ease }}>
                <span
                  className="text-[10px] text-muted-foreground/100 px-3 py-2 rounded-xl font-mono tracking-wide"
                  style={{ background: "hsl(240 5% 7% / 0.75)", border: "1px solid hsl(0 0% 100% / 0.1)" }}
                >
                  {step}
                </span>
                {i < flowSteps.length - 1 && (
                  <ChevronRight className="w-3 h-3 text-muted-foreground/98" />
                )}
              </motion.div>
            ))}
          </div>
          ...
          >
            <div className="flex items-center gap-2">
              ...
              <span className="text-[9px] text-muted-foreground/100 font-mono">loop continuo</span>
        ...
        {/* ─── 10 LAYERS ─── */}
        <div className="mb-24">
          <div className="text-[9px] text-muted-foreground/100 tracking-[0.3em] uppercase text-center mb-8 font-mono">10 LAYER ARCHITETTURALI</div>
              ...
              >
                <div className="p-5 flex items-start gap-4">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-[11px] text-muted-foreground/98 font-light mb-3">{layer.subtitle}</p>
                    ...
                    <div className="flex items-center gap-1.5">
                      <span className="text-[7px] text-muted-foreground/100 font-mono tracking-widest">CAPABILITIES</span>
                      {layer.capabilities.map((cap) => (
                        <span key={cap} className="text-[8px] text-muted-foreground/100 font-mono">{cap}</span>
                      ))}
                    </div>
                  </div>
                </div>
        ...
        <motion.div
          ...
          className="mb-24"
        >
          <div className="text-[9px] text-muted-foreground/100 tracking-[0.3em] uppercase text-center mb-3 font-mono">TOOL CONTRACTS</div>
          <p className="text-[13px] text-foreground/100 font-light text-center mb-8 max-w-lg mx-auto">
          ...
          <div className="rounded-2xl overflow-hidden" style={{ background: "hsl(240 5% 6% / 0.65)", border: "1px solid hsl(0 0% 100% / 0.1)" }}>
            ...
            <div className="flex items-center px-5 py-3 border-b border-border/[0.12]">
              <span className="text-[9px] text-muted-foreground/100 font-mono tracking-wider flex-1">TOOL</span>
              <span className="text-[8px] text-muted-foreground/100 font-mono tracking-wider w-32 text-right">LAYER</span>
              <span className="text-[8px] text-muted-foreground/100 font-mono tracking-wider w-20 text-right">TYPE</span>
            </div>
            {toolContracts.map((tool, i) => (
              ...
              >
                ...
                <span className="text-[9px] text-muted-foreground/100 font-mono w-32 text-right">{tool.layer}</span>
        ...
        <motion.div
          ...
          className="mb-24"
        >
          <div className="text-[9px] text-muted-foreground/100 tracking-[0.3em] uppercase text-center mb-8 font-mono">DUALISMO ARCHITETTURALE</div>
              ...
              <div className="space-y-3">
                {[
                  ...
                  <motion.div key={i} initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 1 + i * 0.1 }} className="flex items-start gap-2">
                    <div className="w-1 h-1 rounded-full bg-primary/90 mt-1.5 flex-shrink-0" />
              ...
              <div className="space-y-3">
                {[
                  ...
                  <motion.div key={i} initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 1.2 + i * 0.1 }} className="flex items-start gap-2">
                    <div className="w-1 h-1 rounded-full bg-accent/90 mt-1.5 flex-shrink-0" />
                    <span className="text-[11px] text-foreground/100 font-light leading-relaxed">{item}</span>
                  </motion.div>
                ))}
              </div>
        ...
        <motion.div
          ...
          className="mb-20"
        >
          <div className="text-[9px] text-muted-foreground/100 tracking-[0.3em] uppercase text-center mb-3 font-mono">MATURITÀ DEL SISTEMA</div>
          <p className="text-[12px] text-muted-foreground/98 font-light text-center mb-8 max-w-sm mx-auto">
              ...
              >
                <div className="flex items-center justify-between mb-2">
                  <div className="flex items-center gap-3">
                    <span className="text-[9px] font-mono text-muted-foreground/100">{String(i + 1).padStart(2, "0")}</span>
                  ...
                  <div className="flex items-center gap-2">
                    <span className={`text-[8px] font-mono px-2 py-0.5 rounded-md ${
                      ...
                      "text-muted-foreground/100 bg-secondary/[0.06]"
                    }`}>
                      {level.status === "live" ? "OPERATIVO" : level.status === "building" ? "IN COSTRUZIONE" : "PIANIFICATO"}
                    </span>
                    <span className="text-[9px] text-muted-foreground/100 font-mono">{level.percent}%</span>
                  </div>
                </div>
                <p className="text-[10px] text-muted-foreground/100 font-light mb-3 ml-7">{level.desc}</p>
                <div className="ml-7">
        ...
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 2.5 }} className="text-center">
          ...
          <p className="text-[10px] text-muted-foreground/100 font-mono tracking-widest">
