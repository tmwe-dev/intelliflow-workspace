  ...
  return (
    <div className="min-h-screen pb-24 flex flex-col items-center px-6 pt-16">
      ...
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8, ease }} className="text-center mb-16 relative z-10">
        <LayoutTemplate className="w-5 h-5 text-primary/30 mx-auto mb-4" strokeWidth={1.5} />
        <h1 className="text-2xl font-light tracking-tight mb-2">Template</h1>
        <p className="text-sm text-muted-foreground/50 font-light">Viste, report e flussi salvati</p>
      </motion.div>
      ...
      <div className="w-full max-w-3xl grid md:grid-cols-2 gap-3 relative z-10">
        {templates.map((t, i) => (
          ...
          >
            ...
            <p className="text-[11px] text-muted-foreground/30">{t.category}</p>
            <div className="flex items-center gap-3 mt-4 text-[10px] text-muted-foreground/25">
