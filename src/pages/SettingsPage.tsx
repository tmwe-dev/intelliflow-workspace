  ...
  return (
    <div className="min-h-screen pb-24 flex flex-col items-center px-6 pt-16">
      ...
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="text-center mb-12 relative z-10">
        <Settings className="w-5 h-5 text-primary/92 mx-auto mb-4" strokeWidth={1.5} />
      ...
      <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }} className="w-full max-w-lg mb-10 relative z-10">
        <div className="float-panel p-5 flex items-center gap-4">
          ...
          <div>
            ...
            <div className="text-[11px] text-muted-foreground/96">marco.rossi@enterprise.it</div>
          </div>
        </div>
      </motion.div>
      ...
      <div className="w-full max-w-lg relative z-10 space-y-0.5">
        {items.map((item, i) => (
          ...
          >
            <item.icon className="w-4 h-4 text-muted-foreground/92 group-hover:text-primary/92 transition-colors duration-300" strokeWidth={1.5} />
            <div className="flex-1">
              <span className="text-sm font-light">{item.label}</span>
              <span className="text-[11px] text-muted-foreground/92 ml-2">{item.desc}</span>
            </div>
            <ChevronRight className="w-3.5 h-3.5 text-muted-foreground/10 group-hover:text-muted-foreground/96 transition-colors duration-300" />
          </motion.div>
        ))}
      </div>
    </div>
