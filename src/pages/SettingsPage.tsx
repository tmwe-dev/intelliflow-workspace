...
const sections = [
  ...
  },
  {
    ...
    items: [
      { icon: Bot, label: "Modelli AI", desc: "Gemini 2.5 Pro (default) · GPT-5 (fallback)" },
      { icon: Activity, label: "Agenti", desc: "6 agenti configurati · Limiti e capability" },
      { icon: Lock, label: "Governance AI", desc: "Conferma richiesta per azioni massive, export e invii" },
    ],
  },
];
  ...
  return (
    <div className="min-h-screen pb-24 px-6 lg:px-10 pt-8 max-w-3xl mx-auto">
      ...
      <motion.div
        ...
        className="glass-panel p-5 flex items-center gap-4 mb-10"
      >
          ...
          >
            ...
            <div className="glass-panel divide-y divide-border/30 overflow-hidden">
              {section.items.map((item) => (
                <div
                  key={item.label}
                  className="flex items-center gap-4 p-4 cursor-pointer hover:bg-secondary/10 transition-colors group"
                >
