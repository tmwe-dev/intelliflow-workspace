  ...
  return (
    <div className="min-h-screen pb-24 flex flex-col items-center px-6 pt-16">
      ...
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8, ease }} className="text-center mb-16 relative z-10">
        ...
        <p className="text-sm text-muted-foreground/98 font-light">Operazioni in corso e completate</p>
      </motion.div>
          ...
          >
            <div className="flex items-start gap-4">
              <div className="mt-0.5 flex-shrink-0">
                {auto.status === "running" ? <Loader2 className="w-4 h-4 text-primary/96 animate-spin" />
                : auto.status === "completed" ? <CheckCircle2 className="w-4 h-4 text-success/95" />
                : auto.status === "approval" ? <ThumbsUp className="w-4 h-4 text-warning/95" />
                : <Pause className="w-4 h-4 text-muted-foreground/100" />}
              </div>
              <div className="flex-1">
                <h3 className="text-sm font-medium mb-1">{auto.name}</h3>
                <p className="text-[11px] text-muted-foreground/100 mb-3">{auto.detail}</p>
                ...
                <div className="flex items-center justify-between">
                  <span className="text-[10px] text-muted-foreground/98 font-mono">{auto.progress}%</span>
                  <span className="text-[10px] text-muted-foreground/98">{auto.agents}</span>
                </div>
              </div>
            </div>
          </motion.div>
