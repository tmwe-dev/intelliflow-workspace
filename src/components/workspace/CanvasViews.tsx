...
export const TableCanvas = ({
  ...
  <CanvasShell onClose={onClose} title={title}>
    ...
    >
      <span className="text-[8px] text-muted-foreground/60 tracking-[0.2em] uppercase font-mono">FONTE</span>
      <span className="text-[9px] text-muted-foreground/60 font-light">Partner DB · Contact Import · Activity Engine · ML Scoring</span>
    </motion.div>

    <table className="w-full">
      <thead>
        <tr className="text-[9px] text-muted-foreground/65 font-mono tracking-wider">
      ...
      <tbody>
        {data.map((row, i) => (
          ...
          >
            ...
            <td className="py-3 text-[12px] text-right text-muted-foreground/65">{row.days} gg</td>
            <td className="py-3 text-right">
              <span className={`text-[11px] font-mono px-2 py-0.5 rounded-lg ${
                ...
                : "text-success/75 bg-success/[0.06]"
              }`}>{row.churn}</span>
            </td>
          </motion.tr>
        ))}
  ...
  <CanvasShell onClose={onClose} title="CAMPAGNA · ANTEPRIMA">
    ...
    >
      <div className="flex items-center justify-between mb-3">
        ...
        <span className="text-[8px] text-muted-foreground/55 font-mono">Email Draft #2847</span>
      </div>
      <div className="text-[11px] text-primary/70 mb-3 font-mono">A: marco.bianchi@techbridge.jp</div>
      <div className="text-[11px] text-foreground/90 mb-3 font-mono">Oggetto: È passato un po', Marco — aggiornamenti per TechBridge</div>
      <div className="text-[12px] text-foreground/90 leading-relaxed font-light space-y-2">
        <p>Gentile Marco,</p>
        ...
        <p className="text-muted-foreground/60 italic">— Bozza generata da Communication Agent · Dati da Contact Memory + Activity Engine</p>
      </div>
    </motion.div>
        ...
        {[
          ...
          <div key={w.wave} className="flex items-center justify-between text-[11px]">
            <div className="flex items-center gap-3">
              <span className="text-foreground/90 font-light">{w.wave}</span>
              <span className="text-muted-foreground/60 font-mono text-[9px]">{w.count}</span>
            </div>
            <div className="flex items-center gap-3">
              <span className="text-muted-foreground/55 text-[9px]">{w.targets}</span>
              <span className="text-muted-foreground/60 font-mono text-[9px]">{w.time}</span>
            </div>
          </div>
        ))}
      </div>
    ...
    <div className="flex items-start gap-3 mt-2">
      <Wand2 className="w-3 h-3 text-primary/70 mt-0.5 flex-shrink-0" />
      <p className="text-[11px] text-muted-foreground/65 leading-relaxed font-light">
...
export const ReportCanvas = ({ onClose }: { onClose: () => void }) => (
  <CanvasShell onClose={onClose} title="REPORT · EXECUTIVE">
    <motion.div
      ...
      className="mb-8"
    >
      <div className="text-[9px] text-muted-foreground/60 tracking-wider uppercase mb-4">PARTNER PERFORMANCE · ASIA PACIFIC · Q1 2026</div>
    ...
    {[
      ...
      >
        ...
        <p className="text-[12px] text-foreground/90 leading-[1.8] font-light">{section.body}</p>
      </motion.div>
    ))}
  ...
  return (
    <CanvasShell onClose={onClose} title="ESECUZIONE · COMPLETATA">
      ...
      >
        ...
        >
          <ThumbsUp className="w-5 h-5 text-success/75" />
        </motion.div>
        <div className="text-lg font-extralight text-foreground/90 mb-2">{config.title}</div>
        <p className="text-[12px] text-muted-foreground/65 font-light">{config.subtitle}</p>
      </motion.div>
          ...
          {config.audit.map((log) => (
            <div key={log.action} className="flex items-center justify-between text-[10px]">
              ...
              <div className="flex items-center gap-2">
                <span className="text-muted-foreground/55 font-mono text-[9px]">{log.agent}</span>
                <span className="text-muted-foreground/55 font-mono text-[9px]">{log.time}</span>
              </div>
            </div>
          ))}
        </div>
  ...
  <div className="h-full flex flex-col rounded-2xl p-6" style={{
    ...
    <div className="flex items-center justify-between mb-6">
      ...
      <div className="flex items-center gap-1.5">
        <button className="text-muted-foreground/55 hover:text-muted-foreground/70 transition-colors duration-500 p-1.5">
          <Download className="w-3 h-3" />
        </button>
        <button onClick={onClose} className="text-muted-foreground/55 hover:text-muted-foreground/70 transition-colors duration-500 p-1.5">
